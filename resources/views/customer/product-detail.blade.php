@extends('customer.index') {{-- layout utama customer --}}
...
@section('content')
    <div class="container py-5">
                ...
                <div class="d-flex">
                    <form action="{{ route('customer.cart.store') }}" method="POST">
                        ...
                        <div class="mb-3 input-group" style="max-width: 220px;">
                            <input type="number" name="quantity" class="px-3 form-control rounded-pill border-primary"
                                value="1" min="1">
                            <button class="px-4 btn btn-primary rounded-pill ms-3">
                    ...
                    <form action="{{ route('customer.wishlist.store') }}" method="POST" class="ml-auto">
                        @csrf
                        <input type="hidden" name="product_id" value="{{ $product->id }}">
                        <button type="submit" class="px-4 btn btn-outline-primary rounded-pill">
        ...
        {{-- Form Review --}}
        <div class="mt-5">
            <h4 class="mb-3 font-weight-bold" style="color:#8B0000;">Tulis Ulasan</h4>
            ...
            <form action="{{ route('customer.review.store', $product->id) }}" method="POST">
                @csrf
                <div class="mb-3">
                    <textarea name="comment" class="rounded form-control" rows="4" placeholder="Tulis ulasan Anda..." required>{{ old('comment') }}</textarea>
                    @error('comment')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                <button type="submit" class="btn btn-primary rounded-pill"><i class="fas fa-paper-plane"></i> Kirim
                    Ulasan</button>
            </form>
        </div>
